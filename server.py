...
# Per-user state — keyed by user id ('shafkat' or 'irfan')
user_state = {
...
# Per-user websocket clients
user_clients = {'shafkat': [], 'irfan': []}

alert_log           = []
current_scan_status = {}   # keyed by user
...
BIOTECH_KEYWORDS = ['fda','trial','phase','clinical','drug','therapy','biotech','pharmaceutical',
    'approval','nda','bla','cancer','oncology']
...
def send_email(subject, body):
    def _send():
        try:
            msg=MIMEMultipart(); msg['From']=EMAIL_ADDRESS; msg['To']=EMAIL_TO; msg['Subject']=subject
            msg.attach(MIMEText(body,'plain'))
            s=smtplib.SMTP('smtp.gmail.com',587); s.starttls()
            s.login(EMAIL_ADDRESS,EMAIL_PASSWORD); s.sendmail(EMAIL_ADDRESS,EMAIL_TO,msg.as_string()); s.quit()
            log_alert(f"📧 Email: {subject}")
        except Exception as e: log_alert(f"⚠️ Email failed: {e}")
...
# ============================================================
# PER-USER MODE & FILTER ENDPOINTS
# ============================================================
...
async def get_stock_metrics(ticker: str):
    try:
        ...
        # Fill missing float & short ratio from yfinance
        if not m.get('float') or not m.get('shortInterestRatio'):
            try:
                info = yf.Ticker(ticker).info
                if not m.get('float'):
                    yf_float = info.get('floatShares',0)
                    if yf_float: m['float'] = round(yf_float/1_000_000, 2)  # in millions
...
async def get_stock_news(ticker: str):
    try:
        today=datetime.now(); from_date=(today-timedelta(days=5)).strftime('%Y-%m-%d'); to_date=today.strftime('%Y-%m-%d')
...
async def search_ticker(ticker: str):
    try:
        ...
        price=quote.get('c',0); prev_close=quote.get('pc',0)
        high=quote.get('h',0); low=quote.get('l',0); open_price=quote.get('o',0)
        ...
        if not price or price==0: return {'error':f'Could not find ticker {ticker}'}
...
def detect_sykes_pattern(ticker,price,prev_close,pct_change,closes,vol_ratio,dollar_vol):
    ...
    if pct_change>=50:
        return("🚀 Supernova",
            ...
            "Sykes: 'Supernovas can last longer and are much greater in magnitude than anybody thinks possible.'",
    ...
    if n>=5:
        recent_high=max(closes[-5:]); recent_run=((recent_high-closes[-5])/closes[-5]*100) if closes[-5]>0 else 0
        ...
        if recent_run>=50 and dip_from_high<=-20 and pct_change>0:
            return("🎯 Morning Panic Dip Buy",
                ...
                "Stop losses cascade creating massive sell-off. Buyers see price as a discount.",
    ...
    if n>=4:
        ...
        if prev_days_red and pct_change>5:
            return("🟢 First Green Day",
                "First green candle after multiple red days. Jack Kellogg's (#1 student, $22M+) all-time favorite. "
    ...
    if pct_change>=15 and vol_ratio>=2:
        return("⚡ Gap and Go",
            ...
            [f"✅ Gapped up {pct_change:.1f}%",
             ...
             "📌 Entry: Buy first pullback after open — never buy the gap open",
             "📌 Exit: Scale out — T1 +10%, T2 +20%, T3 +30%",
             "⚠️ Risk: Gap fills happen fast — stop below gap support"])

    if n>=5 and pct_change>=5:
        recent_max=max(closes[-5:]); flagpole=((recent_max-closes[-5])/closes[-5]*100) if closes[-5]>0 else 0
    ...
    if n>=7:
        mid=n//2; left_high=max(closes[:mid]) if mid>0 else 0
    ...
    if 13<=now_hour<=16 and pct_change>=5 and vol_ratio>=1.5:
        return("🌆 Afternoon Breakout",
            ...
            "Sykes: 'If it breaks HOD with volume, it can explode quickly.'",
        ...
        if dip1<=-10:
            return("📉 Double Bottom Morning Panic",
                ...
                [f"✅ First panic: {dip1:.0f}%",
                 ...
                 "📌 Level 2: Wall of buyers at second bottom",
        ...
        if prev_was_big_up:
            return("🔴 Supernova Fade (Short)",
                ...
                [f"✅ Down {abs(pct_change):.1f}% after big run",
                 ...
                 "📌 Short into bounces — sell the rips",
                 "📌 Cover into panics",
    ...
    if pct_change>=10:
        return("📊 Momentum Play",
            ...
            "Trade with caution — check all 7 Sykes indicators.",
...
def get_massive_gainers(filters):
    try:
        r=requests.get(f"https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/gainers"
        ...
        if r.status_code==200:
            ...
            log_alert(f"📡 Massive: {len(candidates)} stocks"); return candidates
        log_alert(f"⚠️ Massive error: {r.status_code}"); return []
    except Exception as e: log_alert(f"⚠️ Massive error: {e}"); return []

def get_yahoo_gainers(filters):
    candidates=[]; headers={'User-Agent':'Mozilla/5.0'}
    for scrId in ['day_gainers','small_cap_gainers']:
        try:
            r=requests.get(f"https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved"
                           f"?formatted=false&scrIds={scrId}&count=50",headers=headers,timeout=10)
    ...
    log_alert(f"📡 Yahoo: {len(candidates)} stocks"); return candidates
...
# ============================================================
# SCANNER LOOP — PER USER
# ============================================================

async def do_scan(user_id):
    s=user_state[user_id]; filters=s['filters'].copy(); mode=s['mode']
    ...
    status={'phase':'done','message':f'✅ {len(results)} setup(s) found','progress':total,'total':total}
    ...
    while s['running']:
        try:
            ...
            await broadcast_to_user(user_id,{
                ...
                'alerts':alert_log[:20],'scan_status':done_status,
...
# ============================================================
# WEBSOCKET — per user via query param
# ============================================================

@app.websocket("/ws")
async def websocket_endpoint(websocket: WebSocket):
    await websocket.accept()
    # Get user from query param: ws://host/ws?user=shafkat
    ...
    try:
        ...
        if websocket in user_clients.get(user_id,[]): user_clients[user_id].remove(websocket)
